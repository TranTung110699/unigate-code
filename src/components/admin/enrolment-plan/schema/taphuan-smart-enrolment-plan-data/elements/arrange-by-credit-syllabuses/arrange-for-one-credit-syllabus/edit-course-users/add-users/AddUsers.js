...
const Add = ({ dialogKey, formid, user, course }) => {
  ...
  return (
    <ApiRequestBtnWithConfirmDialog
      textConfirm={t1('do_you_want_to_add_this_user_to_course?')}
      formidToSubmitOnSuccess={formid}
      url={taphuanSmartEnrolmentPlanApi.add_course_users}
      params={{
        user_iid: userIid,
...
const AddUsers = ({ course, dialogKey }) => {
  const courseIid = lodashGet(course, 'iid');

  const formid = `add_course_users_${courseIid}`;
  ...
  return (
    <SearchWrapper
      ...
      alternativeApi={
        taphuanSmartEnrolmentPlanApi.search_users_to_add_to_course
      }
      hiddenFields={{
        course_iid: courseIid,
      }}
