...
class Layout extends Component {
  ...
  renderResultComponent(items, props) {
    const renderResults = (showEditAccountButton) => (
      <Results
        items={items}
        {...props}
        searchFormId="hrms_phongban_search"
        form="hrms_phongban_search_result"
  ...
  render() {
    return (
      <div>
        <SearchWrapper
          formid={'hrms_phongban_search'}
          schema={schema()}
          renderResultsComponent={this.renderResultComponent}
          showSearchButton
          alternativeApi={hrmsEvnApiUrls.hrms_phongban_search}
          searchResultKey={`hrms_phongban_search`}
        />
      </div>
    );
  }
