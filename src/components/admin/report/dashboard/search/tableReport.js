...
import UserResult from 'components/admin/path/classgroup/users/Results';
import {
...
class Report extends React.Component {
  ...
  render() {
    ...
    if (themeConfig && themeConfig.type === 'sis') {
      return (
        <div className="m-t-10">
          <UserResult {...this.props} ntype="course" />
        </div>
      );
    }
      ...
      <div>
        <div style={this.divStyle} className="row">
          <div className="col-md-12">
            {groups && groups.length && !notShowReportName && (
              <h3>
                {t1('report_in_groups')}: {nameReport}
              </h3>
            )}
            {/*
